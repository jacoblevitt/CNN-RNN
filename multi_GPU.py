...
# empty lists to hold semi-processed data and calibrate padding, and prediction scoring
def initialize():
    global trimmed, unnested, x_lengths, y_lengths, correct, score
    trimmed = []
    unnested = []
    x_lengths = []
    y_lengths = []
...
def predict(x):
    targets = expand(targets, char2id_x, maxlen_x, vocab_size_x)
    target_input_fn = lambda: tf.data.Dataset.from_tensor_slices(targets).batch(1)
    predictions = estimator.predict(input_fn = target_input_fn)
    for i in range(len(x)):
        prediction = next(predictions)
        prediction = [prediction.popitem()[1]]
        prediction = ''.join(decode(prediction).tolist()).split('\n')[0]
        prediction = prediction.replace('L', 'Cl').replace('R', 'Br').replace('E', 'Se')
        print(prediction)
        try:
            mol = Chem.MolFromSmiles(prediction)
            correct.append(1)
            fingerprint = AllChem.GetMorganFingerprint(mol, 3)
            for j in inhibitors:
                j = Chem.MolFromSmiles(j)
                j = AllChem.GetMorganFingerprint(j, 3)
                j = DataStructs.DiceSimilarity(j, mol)
                score.append(j)
        except:
            pass
...
# construct our target dataset -- the human kinome
target_file = open('human_protein_sequences.fasta').readlines()
target_list = [i for i in target_file if '>' not in i]
target_df = pd.DataFrame(target_list, columns = ['Sequence'])
target_df['Sequence'] = target_df['Sequence'].str.replace('\n','')
targets = target_df.values.tolist()
targets = sum(targets, [])
# comparing predictions to known kinase inhibitors
inhibitor_df = pd.read_csv('kinase_inhibitors.csv')
inhibitors = inhibitor_df.values.tolist()
# remove our prediction data from the training set
data = data[~data.Sequence.isin(target_df.Sequence)]
data = data[~data.SMILES.isin(inhibitor_df.SMILES)]
data = shuffle(data)
print(data)
...
# shuffles the dataset and feeds to estimator in batches of 256 for 5000 epochs
train_input_fn = lambda: tf.data.Dataset.from_tensor_slices((x_data_train, y_data_train)).shuffle(10000).batch(256).repeat(10000)
eval_input_fn = lambda: tf.data.Dataset.from_tensor_slices((x_data_test, y_data_test)).shuffle(10000).batch(256).repeat(10000)
train_spec = tf.estimator.TrainSpec(input_fn = train_input_fn)
eval_spec = tf.estimator.EvalSpec(input_fn = eval_input_fn)
tf.estimator.train_and_evaluate(estimator, train_spec, eval_spec)
# interested in how many share common substructure 
predict(targets)
print('Total Correct: %s' % sum(correct))
print('Total Score: %s' % sum(score))
